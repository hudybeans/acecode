...
final class InstallerEngine {
    ...
    func install(source: URL, folder: URL, replace: Bool, scope: InstallScope = .personal,
        ...
        let lock = directory.appendingPathComponent(".acecode-install.lock")
        var fd = open(lock.path, O_CREAT | O_EXCL | O_RDWR | O_NOFOLLOW, mode_t(0o644))
        if fd < 0 && errno == EEXIST { fd = open(lock.path, O_RDONLY | O_NOFOLLOW) }
        guard fd >= 0 else { throw InstallError(message: "无法创建安装锁，请检查目录权限。") }
        defer { close(fd) }
        guard flock(fd, LOCK_EX | LOCK_NB) == 0 else { throw InstallError(message: "另一个安装程序正在使用此目录。") }
