...
@main struct Tests {
    static func main() throws {
        ...
        let root = fm.homeDirectoryForCurrentUser.appendingPathComponent(".acecode-installer-test-" + UUID().uuidString)
        ...
        try check("concurrent lock rejected") {
            let fd = open(folder.appendingPathComponent(".acecode-install.lock").path, O_RDWR)
