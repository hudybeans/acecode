...
TEST(DesktopInstanceStartup, InjectedIdentityOnlyAppliesToAdditionalInstances) {
    const auto primary = plan_instance_startup(false, true, "owner-a", "acecode-f33bbdb9cef6");
    EXPECT_EQ(primary.run_subdirectory, "desktop-shared");
    const auto extra = plan_instance_startup(true, false, "owner-a", "acecode-f33bbdb9cef6");
    EXPECT_EQ(extra.run_subdirectory, "desktop-instances/acecode-f33bbdb9cef6-owner-a");
    const auto concurrent = plan_instance_startup(true, false, "owner-b", "acecode-f33bbdb9cef6");
    EXPECT_NE(extra.run_subdirectory, concurrent.run_subdirectory);
    EXPECT_EQ(plan_instance_startup(true, false, "owner-a", "../escape").run_subdirectory,
              "desktop-instances/owner-a");
}
