        ...
        while (std::chrono::steady_clock::now() < deadline) {
            {
                ...
                if (state.ask_pending) {
                    opened = true;
                    // The real overlay records answers by the rendered question
                    // text, not the tool's generated wire ID.
                    const auto& question = state.ask_questions.at(0);
                    state.ask_result_answers[question.question] = question.options.at(0).label;
