...
- **三套机制**(`src/tool_preamble/` 纯逻辑 + AgentLoop 接线):
  - `prompt`:给每个工具定义注入 `preamble` 字符串参数,系统提示追加「# Tool call preamble」段要求每次调用都填一句(「不要叙述工具调用」的既有口径保留);参数流式前缀里一抽到值就当 tool_planning 的 label,落盘 / 执行前从参数里剥掉,前言经 `tool_start.preamble` 与 `metadata.tool_preamble.calls` 传给界面。(第一版「先说一句话再调工具」已废:文本先变气泡、批次开始才搬进 loading、连续单工具步堆成一摞标题行。)
