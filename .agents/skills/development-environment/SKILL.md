...
- `ACECODE_DESKTOP_INSTANCE_ID` —— 取 `worktree 名-commit 前 12 位`（经字符净化），作为稳定的开发身份前缀。桌面进程始终保留独立的 owner UUID，附加实例使用 `run/desktop-instances/<identity>-<owner_uuid>`；同一工作树和提交的并发实例不会共用目录或清理对方的 daemon。切 commit 时身份前缀随之变化。
