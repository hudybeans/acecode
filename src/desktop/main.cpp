...
int main(int argc, char** argv) {
    ...
    // bridge: get/set home workspace selector preference.
    host.bind("aceDesktop_getHomeWorkspaceHash", [&](const std::string& /*req*/) -> std::string {
        return nlohmann::json{
            {"workspace_hash", acecode::read_last_home_workspace_hash()}
        }.dump();
    });
    host.bind("aceDesktop_setHomeWorkspaceHash", [&](const std::string& req) -> std::string {
        try {
            auto arr = nlohmann::json::parse(req);
            if (!arr.is_array() || arr.empty() || !arr[0].is_string()) {
                return nlohmann::json{{"error", "missing hash arg"}}.dump();
            }
            acecode::write_last_home_workspace_hash(arr[0].get<std::string>());
            return nlohmann::json{{"ok", true}}.dump();
        } catch (const std::exception& e) {
