...
ToolDef exit_worktree_def() {
    ...
    def.description =
        ...
        "\n"
        "When to use: the user explicitly asks to exit / leave the worktree, go "
        "back, return to the main checkout, or merge/land the worktree onto "
        "master/main. A git merge or checkout is NOT an exit — the Desktop "
        "worktree badge stays until this tool runs. After merging, call this "
        "tool (usually action=keep) so the session cwd and UI leave the worktree. "
        "Do NOT call this just because a feature is finished.\n"
        "\n"
