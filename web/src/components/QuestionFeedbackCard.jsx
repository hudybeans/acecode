...
export function QuestionFeedbackCard({ feedback }) {
  ...
  if (interjected) {
    return (
      ...
      >
        <div className="flex items-center gap-2 px-3.5 py-2.5 text-[12px] leading-normal text-fg-mute">
    ...
    >
      <div>
        <div className="flex items-center gap-2 px-3.5 py-2.5">
          ...
          <h3 className="text-[13px] leading-normal font-medium text-fg">{cancelled ? '已取消全部回答' : '全部提交完成'}</h3>
        </div>
        {summary.length > 0 && (
          <ul className="border-t border-border">
            {summary.map((item, index) => (
              <li key={index} className="border-t border-border px-3.5 py-3 first:border-t-0">
                <div className="text-[11px] leading-normal text-fg-mute break-words">
                  {index + 1}. {item?.question}
                  {item?.multiSelect ? '（多选）' : ''}
                </div>
                <div className="mt-0.5 text-[13px] leading-normal break-words">
                  {item?.notAnswered ? (
