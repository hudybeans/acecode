...
run('selects only eligible composer or visible terminal targets', () => {
  assert.deepEqual(nativeFileDropTarget(element({ composer: { dataset: {} } })), { kind: 'composer' });
  const nestedOverlay = {
    closest(selector) {
...
run('routes a coordinate-bearing drop to composer exactly once', () => {
  ...
  const win = {
    innerWidth: 1000,
    innerHeight: 500,
    __aceComposerAcceptFileDrop: (payload) => calls.push(['composer', payload]),
  ...
  const doc = {
    elementFromPoint: (x, y) => {
      assert.equal(x, 500);
      assert.equal(y, 125);
      return element({ composer: { dataset: {} } });
    },
  };
