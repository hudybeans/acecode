...
run('架构: pill 读 git info 走共享缓存,且不可渲染时不发请求', () => {
  ...
  assert.match(pillSource, /gitInfoCache\.peek\(api, cwd, owner\)/);
  assert.match(pillSource, /gitInfoCache\.get\(api, target, owner\)/);
  assert.match(pillSource, /gitInfoCache\.refresh\(api, target, owner\)/);
  // 不许绕过缓存直接打接口
