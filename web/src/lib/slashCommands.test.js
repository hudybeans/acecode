...
run('description 子串匹配在 name 不命中时仍然被收录', () => {
  // “历史”只在 compact 的本地化 description 里出现 → 该项分数 100
  const r = rankCommands('历史', ITEMS);
...
await runAsync('已知 builtin 元数据随语言切换，未知后端文案保持原样', async () => {
  try {
    await applyLocalePreference('en-US', { cache: false, effectiveLocale: 'en-US' });
    const items = flattenCommands({
      builtins: [
        { name: 'init', description: '后端自定义 init 文案' },
