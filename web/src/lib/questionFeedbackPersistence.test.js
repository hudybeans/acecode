...
import { renderQuestionFeedbackCardForTest } from './questionFeedback.test.js';
...
await run('缺少调用或工具被改名的取消结果不会折叠进历史活动', () => {
  for (const toolName of ['', 'request_input']) {
    ...
    assert.match(renderQuestionFeedbackCardForTest(feedbackItems[0]), /data-question-feedback="cancel"/);
...
await run('历史问答保留多选标记和完整答案', () => {
  ...
  assert.match(renderQuestionFeedbackCardForTest(item), /A, B\nC/);
});
