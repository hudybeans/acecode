...
在 Web 界面按 **Ctrl+O**，播放与 `demo/index.html` 中「测试完整动画」相同的一轮效果：彩边渐入、底部中央热浪、短暂停留、渐出关闭，默认约三秒。文字和弹窗一起轻微扭曲，输入焦点和草稿保持不变。

默认不运行，播放时再次触发不会叠加，结束后停止绘图并释放资源。页面隐藏时暂停；系统开启减少动态效果时，使用较短过渡和较轻形变。
