...
所有背景图片保持长宽比例。首页/会话图默认 cover 铺满对应区域；会话图不限制在居中横幅。用户消息图按气泡宽度等比缩放并底部对齐，不重复；气泡超出图片高度的部分显示 user_message_background_color。素材顶部底色必须与该颜色一致，或用透明装饰露出它，避免接缝。图像透明度只作用于图层，不淡化消息文字。用户约定的素材导出尺寸记录在需求中，不新增未支持的 appearance 尺寸键。
