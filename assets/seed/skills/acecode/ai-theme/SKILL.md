...
license: MIT
metadata:
  source_id: acecode:ai-theme@2026-09-15.3
...
9. **所有主题图片只允许等比缩放。** 会话背景默认铺满右侧会话区域（cover），不制作带上下留边的横幅；只有用户明确约定尺寸或构图时才按约定处理。所有用户消息背景统一显示为 720 CSS px 宽、高度按比例自动计算（720px auto），右下对齐（right bottom）、不重复。短消息、长消息及宽窄窗口中的图案大小保持一致，只按气泡边界裁切；长消息高出图片的部分由同色底色补齐。
10. **代码阅读区域默认高对比：浅色背景必须配深色文字，深色背景必须配浅色文字。** 以代码区域实际底色判断，不以整套主题的 mode 或壁纸判断。主题主色、柔和风格或低饱和偏好不构成例外；只有用户明确坚持打破此规则才可在其指定范围内例外，处理方式见 [代码阅读舒适度](references/theme-contract.md#代码阅读舒适度)。
...
确认前单独展示代码区域的底色和文字色，检查明暗方向及对比度；同时检查代码卡片使用的 surface/fg。代码区不必与整体主题同为浅色或深色，优先保证持续阅读舒适。
...
用户消息素材的叶子、花纹等装饰安排在右下角，按统一 720 CSS px 显示宽度设计；短消息只看到底图右下部分，不能为容纳完整图案而单独缩小。顶部使用与 `user_message_background_color` 一致的纯色，或将装饰背景设为透明以露出该底色，避免长消息出现色差接缝。必须预览短消息、长消息及窄窗口；检查右下装饰位置稳定、所有消息中的图案大小一致且不变形、超高部分同色衔接。背景色有改动仍保留其它 appearance 参数并重新确认。

HTML 预览主页在上、聊天页在下；①主页、②会话页、③仅用户消息，使用与安装相同的颜色和 opacity。核对图标色、首页标题色和通顶；深色通顶右侧按钮为白色，浅色通顶使用主题前景，顶部背景必须可读。

代码可读性必须用真实代码文本检查：普通文字、关键词、字符串、数字、注释、行号和选中文字都应清晰。不能只看壁纸或用一张重绘截图宣称已验证实际高亮；按主题契约核对当前渲染器支持的颜色范围。
