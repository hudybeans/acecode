...
- **三种来源的差别**:`prompt` 改系统提示口径(`build_system_prompt` 尾参 `prompt_tool_preamble`,把「Do not narrate every tool call」换成 Codex 式 preamble;关闭时逐字节不变,`system_prompt_tool_preamble_test` 守着),Web 把那句正文折进分组不当气泡,TUI 把刚流完的正文行原地改成标题行;`reasoning` 在流回调里抠加粗,流式期间 `agent_progress{reasoning}` 的 label 就换成标题(TUI 经 `on_thinking_title` 替换等待动画短语),Codex app-server(`item/reasoning/summaryTextDelta`)与 Grok Responses 已带回摘要,Anthropic / DeepSeek 只能首句兜底;`sidecar` 在**第一个完整** ToolCall 露头时起 detached 线程调注入的摘要器(`set_tool_preamble_sidecar_summarizer`,SessionRegistry / main.cpp 各接一份,复用自动标题那套一次性 provider,流超时封顶 8s),落盘前有界等 `sidecar_wait_ms`,等不到就无标题落盘、工具执行完 / 回合末补发 `late:true` 事件**只更新界面不写 JSONL**(已落盘消息没有改写入口)。摘要线程只写任务对象,AgentLoop 轮询、永不被回调,避免析构后 UAF。
- **loading 提示同源**:有标题时 `agent_progress` 的 tool_planning / tool_running 的 label = 标题(工具名 / 命令预览退到 detail),所以只读 `activity.label` 的消费方(侧栏、迷你视图)也显示标题;Web `ActivitySummaryBlock` 里 `item.preamble.title` > 阶段文案 / 并行计数。
- **Web 分组键是标题不是 batch_id**:实时 Message 帧的 metadata 只有 title/source,拿 batch_id 当键会把 prompt 正文条目和它的工具项拆成两组;group id 才用 batch_id 保证流式追加稳定。reducer 先把事件暂存在 `pendingToolPreambles`(按 tool_call_id),`tool_start` 建项时取走;历史加载由 `tagHistoryToolPreambles` 从 assistant metadata 传播到同批次结果项(结构化结果挂 `tool.preamble`,legacy 文本结果挂 metadata,`makeLegacyInvocationItem` 归并时带上)。无标题的工具项跟在带标题批次后要另起无标题组(`splitPreambleGroups`),否则会被上一组标题冒领;完全没标题时 `flushToolBuffer` 走原路径,投影与改动前一致。`tool_preamble` 在 `STREAM_NEUTRAL_EVENT_TYPES` 里,不切断流式草稿。
