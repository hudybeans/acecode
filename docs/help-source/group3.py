...
"model-basics": page("先区分服务商、模型和已保存预设，才能正确选择连接方式并在任务中切换模型。", [
    section("concepts", "三个名称分别表示什么",
        table(["名称", "含义", "例子"], [["服务商 / Provider", "决定请求协议、连接地址、认证方式及可配置字段。", "ACEModel、GitHub Copilot、自定义 OpenAI 兼容接口。"], ["Model ID", "服务端识别的模型名称，必须与接口实际支持的 ID 一致。", "从目录或探测结果选择，也可以手动填写。"], ["预设名称（表单中叫别名）", "你保存这组连接配置时使用的名称。表单会默认填入选中模型的 ID，可改成便于辨认的名字。", "日常编程、项目审查等便于自己辨认的名字。"]]),
    ...
    section("setup", "从目录到任务模型",
        '''<ol><li>进入<strong>设置 &gt; 模型</strong>，点击<strong>新增模型</strong>。</li><li>选择 Provider。自定义接口、需要 API Key 的服务商、设备登录服务商的表单不同。</li><li>填写连接信息，选择或输入 Model ID，按需修改自动填入的别名。</li><li>保存模型，回到任务，在输入框下方选中这个预设。</li><li>发送简短请求验证连接，再开始正式代码任务。</li></ol>''',
...
"acemodel": page("ACEModel 是 ACECode 内置目录中的服务商入口。选定模型并保存连接后，就可以在任务中使用。", [
    ...
    section("connect", "保存并开始使用",
        '''<ol><li>进入设置的模型页并点击新增模型。</li><li>选择 ACEModel，确认表单中的连接地址。</li><li>按账号获得的连接信息填写 API Key，选择需要的模型。</li><li><strong>别名</strong>默认取选中模型的 ID，与已有预设重名时自动追加 (1)、(2)，可按需改成便于辨认的名字。</li><li>保存后在任务模型选择器中选中它，发送一次简短请求。</li></ol><p>不清楚某项限制时，先保留界面提供的参数，再核对服务实际支持的配置。不要为了消除超限报错随意增大上下文窗口。</p>''',
...
"manage-models": page("管理可供任务使用的模型预设，并区分新任务默认值与当前任务的模型选择。", [
    section("crud", "添加、编辑与删除",
        '''<p>模型页的<strong>已保存模型</strong>列表支持搜索、刷新和新增。每条预设显示名称、Provider、Model ID 与能力标识，可编辑、设为默认或删除。</p><ol><li><strong>添加：</strong>点击新增模型，选择 Provider 和 Model ID，填写连接信息，保存模型。</li><li><strong>批量添加：</strong>新增时，支持多选的模型目录或探测窗口可选择多个 ID；此时别名字段变为<strong>别名前缀</strong>（默认为服务商名），每个模型保存为“前缀-Model ID”，重名自动追加 (1)、(2)。</li><li><strong>编辑：</strong>打开已有预设，修改需要的字段，再点击保存修改。编辑时保留原有 Provider 身份。</li><li><strong>删除：</strong>核对弹窗中的名称后删除预设；会话记录仍保留。</li></ol>''',
        '''<p>已保存密钥不会明文回填。编辑时不填写新密钥通常保留原凭据；需要删除凭据时勾选<strong>清除已保存密钥</strong>。新增时的<strong>别名</strong>会随所选模型自动填入，手动改过后不再被覆盖。</p><p>名称冲突时，界面提供覆盖已有预设、另存为或取消。不要为绕过冲突随意覆盖还在使用的连接。运行中的会话使用该预设时，删除可能显示<strong>会话使用中</strong>并被阻止，先结束或切换相关任务。</p>''',
